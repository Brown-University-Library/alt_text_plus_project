# 50 human-reviewed, CC0 images with published alt text

Research date: 2026-08-12  
Length-cap update: 2026-08-12

## Contents

- [Selection standard](#selection-standard)
- [How character counts were calculated](#how-character-counts-were-calculated)
- [Why this source qualifies](#why-this-source-qualifies)
- [License and reuse](#license-and-reuse)
- [Coverage](#coverage)
- [The 50 results](#the-50-results)
- [Research notes and exclusions](#research-notes-and-exclusions)
- [Source documentation](#source-documentation)
- [Clarifications supplied](#clarifications-supplied)
- [Prompt](#prompt)


## Selection standard

Every result below had to meet all of these conditions:

1. The link resolves to a specific image, not just a search page.
2. The image's museum record is explicitly marked CC0.
3. The exact published description is attached to that specific primary image and is used as its screen-reader-accessible alt text.
4. The image's published metadata says `human_reviewed: true`.
5. The description stands on visual content rather than depending on an article, lesson, or other surrounding context to make sense.
6. The normalized description is no more than 200 characters, treating 200 as an inclusive maximum.

The descriptions are reproduced exactly apart from collapsing line breaks and repeated spaces. All 12 entries from the earlier report that met the new limit were retained; 38 longer entries were replaced with other qualifying records.

## How character counts were calculated

Each count includes every displayed letter, number, space, and punctuation mark in the reproduced alt text. Before counting, line breaks and repeated whitespace were collapsed to single spaces, and whitespace at the beginning or end was removed. The quoted text and the count therefore correspond to each other. The maximum for this sample is 200 characters inclusive.

## Why this source qualifies

All 50 results come from the Cleveland Museum of Art's image-description initiative. This is stronger evidence than finding ordinary alt text on a museum website:

- The museum developed its standard with [Prime Access Consulting](https://www.pac.bz/), an accessibility firm led by disabled experts.
- The museum and the accessibility consultants spent months defining a high-quality visual description and produced a [collection-specific style guide](https://static.sched.com/hosted_files/mcn2024/15/CMA%20Collection%20Online%20Alt-Text%20Style%20Guide.pdf).
- Museum staff first wrote descriptions themselves and refined them in response to the consultants' feedback.
- The publishing system distinguishes descriptions that received human review from descriptions that did not. This report includes only records whose primary-image metadata says `human_reviewed: true`.
- The descriptions are attached to specific images, rather than merely to artwork records, and the museum uses them as screen-reader alt text.

This does not mean that every reader will prefer every wording. It means there is documented, accessibility-informed human judgment behind the selected descriptions—the standard requested for this search.

## License and reuse

Every source image link below is from the museum's Open Access image service, and every corresponding record reports `share_license_status: "CC0"`. The museum says its CC0 images may be downloaded, shared, changed, and reused without asking permission. Attribution is not legally required for CC0 material, but retaining the museum record link is useful for provenance.

CC0 addresses copyright. A user should still consider non-copyright issues that may apply to a particular use, such as privacy, publicity, cultural sensitivity, or trademarks.

All source image and museum-record links were checked on 2026-08-12 and returned successfully.

## Coverage

The selection deliberately favors breadth over fifty similar paintings. It covers 17 object/image types: calligraphy, carpet, ceramic, drawing, furniture and woodwork, ivory, jade, jewelry, lacquer, metalwork, miscellaneous designed objects, painting, photograph, print, sculpture, silver, and wood.

It spans 15 museum areas: American Painting and Sculpture; Art of the Americas; Chinese Art; Decorative Art and Design; Drawings; Egyptian and Ancient Near Eastern Art; European Painting and Sculpture; Greek and Roman Art; Japanese Art; Korean Art; Medieval Art; Modern European Painting and Sculpture; Photography; Prints; and Textiles.

## The 50 results

### 1. Raft Cup

- Type: Silver — Chinese Art
- Creator/date: Zhu Bishan (Chinese, c. 1300–after 1362); 1300s–1400s
- License: CC0
- Links: [local image](curated_images/1977.7_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1977.7/1977.7_web.jpg) · [museum record](https://www.clevelandart.org/art/1977.7)
- Character count: 195
- Published alt text:

> A silver cup is shaped like a man riding down a river. The man has a naked torso and sits at the lip of the cup, his head tilted back. The waves envelop each side of the cup and its upturned tip.

### 2. Plaque

- Type: Jewelry — Art of the Americas
- Creator/date: Unknown; c. 500–200 BCE
- License: CC0
- Links: [local image](curated_images/1938.431_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1938.431/1938.431_web.jpg) · [museum record](https://www.clevelandart.org/art/1938.431)
- Character count: 116
- Published alt text:

> A small gold plaque depicts a face with fangs, framed by many serpent heads. The plaque is flat with raised designs.

### 3. Battle of the Nudes

- Type: Print — Prints
- Creator/date: Antonio del Pollaiuolo (Italian, 1431/32–1498); 1470s–80s
- License: CC0
- Links: [local image](curated_images/1967.127_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1967.127/1967.127_web.jpg) · [museum record](https://www.clevelandart.org/art/1967.127)
- Character count: 175
- Published alt text:

> A horizontally oriented engraving depicts many muscular nude men violently fighting, some with swords, daggers, and other various weapons. In the background, there is foliage.

### 4. Royal Carpet with Silk and Metal Thread

- Type: Carpet — Textiles
- Creator/date: Unknown; 1600–1625
- License: CC0
- Links: [local image](curated_images/1926.533_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1926.533/1926.533_web.jpg) · [museum record](https://www.clevelandart.org/art/1926.533)
- Character count: 164
- Published alt text:

> A large, vertically long carpet features elaborate floral patterns in shades of orange, blue, green, and brown. It is trimmed with fringe on the two shortest sides.

### 5. Sacrificer Container

- Type: Wood — Art of the Americas
- Creator/date: Unknown; 770–890 (radiocarbon date, 95% probability)
- License: CC0
- Links: [local image](curated_images/2007.193_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2007.193/2007.193_web.jpg) · [museum record](https://www.clevelandart.org/art/2007.193)
- Character count: 194
- Published alt text:

> A small wood figure with the face of a feline bares its teeth. It draws a knife across the throat of a human in its lap. The entire surface of the figure has been carved with geometric patterns.

### 6. Ivory Plaque with Enthroned Mother of God ("The Stroganoff Ivory")

- Type: Ivory — Medieval Art
- Creator/date: Unknown; 950–1025
- License: CC0
- Links: [local image](curated_images/1925.1293_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1925.1293/1925.1293_web.jpg) · [museum record](https://www.clevelandart.org/art/1925.1293)
- Character count: 190
- Published alt text:

> A vertically oriented carved ivory relief depicts a woman seated in the center holding a child in her lap. Two angels hover above, one in each upper corner. There are letters carved beneath.

### 7. Desk and Bookcase

- Type: Furniture and woodwork — Decorative Art and Design
- Creator/date: John Townsend (American, 1732–1809); c. 1780–95
- License: CC0
- Links: [local image](curated_images/2012.43_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2012.43/2012.43_web.jpg) · [museum record](https://www.clevelandart.org/art/2012.43)
- Character count: 154
- Published alt text:

> A tall reddish-brown wood bookcase features many drawers and components. It is detailed with simple but refined designs and shiny brass knobs and handles.

### 8. Gamin

- Type: Sculpture — American Painting and Sculpture
- Creator/date: Augusta Savage (American, 1892–1962); c. 1929
- License: CC0
- Links: [local image](curated_images/2003.40_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2003.40/2003.40_web.jpg) · [museum record](https://www.clevelandart.org/art/2003.40)
- Character count: 194
- Published alt text:

> A realistic, hand-painted plaster sculpture depicts a Black boy's head and top half of his torso. The sculpture looks like bronze and the subject wears a flat cap and looks slightly to our left.

### 9. Feast Ladle

- Type: Miscellaneous — Art of the Americas
- Creator/date: Unknown; late 1800s–early 1900s
- License: CC0
- Links: [local image](curated_images/1953.386_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1953.386/1953.386_web.jpg) · [museum record](https://www.clevelandart.org/art/1953.386)
- Character count: 187
- Published alt text:

> A ladle features a large light brown spoon and a long dark brown handle. The top of the ladle's handle is an eagle head. The bottom of the spoon has been carved to resemble a bear's face.

### 10. Head Fragment

- Type: Jade — Art of the Americas
- Creator/date: Unknown; c. 900–300 BCE
- License: CC0
- Links: [local image](curated_images/1961.31_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1961.31/1961.31_web.jpg) · [museum record](https://www.clevelandart.org/art/1961.31)
- Character count: 184
- Published alt text:

> A small light green segment of a head features the face of a figure from below the eyes to the top of the neck. There is a hole in each earlobe and the figure's mouth is slightly open.

### 11. Bell (Lai Zhong)

- Type: Metalwork — Chinese Art
- Creator/date: Unknown; c. 800–700 BCE
- License: CC0
- Links: [local image](curated_images/1989.3_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1989.3/1989.3_web.jpg) · [museum record](https://www.clevelandart.org/art/1989.3)
- Character count: 146
- Published alt text:

> A bronze bell features a green surface, engraved with designs and inscriptions. Three rows of six small knobs protrude from each side of the bell.

### 12. Arm of Eve

- Type: Drawing — Drawings
- Creator/date: Albrecht Dürer (German, 1471–1528); 1507
- License: CC0
- Links: [local image](curated_images/1965.470_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1965.470/1965.470_web.jpg) · [museum record](https://www.clevelandart.org/art/1965.470)
- Character count: 104
- Published alt text:

> A vertically oriented drawing with gray tones of an arm, bent at the elbow, holds up a spherical object.

### 13. Sand Dunes, Carson Desert, Nevada

- Type: Photograph — Photography
- Creator/date: Timothy H. O'Sullivan (American, 1840–1882); 1867
- License: CC0
- Links: [local image](curated_images/2002.45_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2002.45/2002.45_web.jpg) · [museum record](https://www.clevelandart.org/art/2002.45)
- Character count: 148
- Published alt text:

> A horizontally oriented photograph depicts vast sand dunes with footprints. On our left, a wagon in the distance is pulled by a team of four horses.

### 14. Cosmetics Box with Chrysanthemums

- Type: Lacquer — Japanese Art
- Creator/date: Unknown; early 1300s
- License: CC0
- Links: [local image](curated_images/1963.513_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1963.513/1963.513_web.jpg) · [museum record](https://www.clevelandart.org/art/1963.513)
- Character count: 140
- Published alt text:

> A black box is painted with small, simple clusters of golden flowers all around. The knob of the box is in the shape of a small gold flower.

### 15. Mount Sainte-Victoire

- Type: Painting — Modern European Painting and Sculpture
- Creator/date: Paul Cezanne (French, 1839–1906); c. 1904
- License: CC0
- Links: [local image](curated_images/1958.21_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1958.21/1958.21_web.jpg) · [museum record](https://www.clevelandart.org/art/1958.21)
- Character count: 98
- Published alt text:

> A colorful oil painting depicts a landscape with green trees and a mountain, all under a blue sky.

### 16. Red-Figure Calyx-Krater (Mixing Vessel): Medea in Chariot (A); Telephos with Baby Orestes (B)

- Type: Ceramic — Greek and Roman Art
- Creator/date: Policoro Painter (South Italian, Lucanian, active c. 400 BCE); c. 400 BCE
- License: CC0
- Links: [local image](curated_images/1991.1_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1991.1/1991.1_web.jpg) · [museum record](https://www.clevelandart.org/art/1991.1)
- Character count: 146
- Published alt text:

> A large black ceramic vessel features handles that curve up. Around the vessel are two scenes in brown ceramic of tragedies dealing with children.

### 17. Selections from the New Collection of Japanese Poems from Ancient and Modern Times (Shinkokin wakashū) with Printed Designs of Chinese Lions and Crab Finger Grass

- Type: Calligraphy — Japanese Art
- Creator/date: Hon'ami Kōetsu (Japanese, 1558–1637); early 1600s
- License: CC0
- Links: [local image](curated_images/1966.118_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1966.118/1966.118_web.jpg) · [museum record](https://www.clevelandart.org/art/1966.118)
- Character count: 108
- Published alt text:

> A handscroll is filled with Japanese calligraphy on top of paintings of tall grass with lions leaping about.

### 18. Julius Caesar

- Type: Sculpture — European Painting and Sculpture
- Creator/date: Mino da Fiesole (Italian, c. 1430–1484); c. 1455–60
- License: CC0
- Links: [local image](curated_images/2009.271_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2009.271/2009.271_web.jpg) · [museum record](https://www.clevelandart.org/art/2009.271)
- Character count: 109
- Published alt text:

> A marble relief depicts the side profile of Julius Caesar, facing our left, surrounded by a circular garland.

### 19. Basin with Inscribed Figures and Calligraphy

- Type: Metalwork — Korean Art
- Creator/date: Unknown; 1100s–1200s
- License: CC0
- Links: [local image](curated_images/1985.112_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1985.112/1985.112_web.jpg) · [museum record](https://www.clevelandart.org/art/1985.112)
- Character count: 135
- Published alt text:

> A shiny round bronze bowl features two small handles. Across the surface of the bowl are silver inlay script and depictions of a child.

### 20. Nome Gods Bearing Offerings

- Type: Sculpture — Egyptian and Ancient Near Eastern Art
- Creator/date: Unknown; c. 1391–1353 BCE
- License: CC0
- Links: [local image](curated_images/1961.205_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1961.205/1961.205_web.jpg) · [museum record](https://www.clevelandart.org/art/1961.205)
- Character count: 200
- Published alt text:

> A painted limestone block features three figures, in profile facing our right, each carrying a tray of many objects and having an animal by their side. The figures are separated by many hieroglyphics.

### 21. Self-Portrait

- Type: Print — Prints
- Creator/date: Edgar Degas (French, 1834–1917); 1857
- License: CC0
- Links: [local image](curated_images/2004.87_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2004.87/2004.87_web.jpg) · [museum record](https://www.clevelandart.org/art/2004.87)
- Character count: 185
- Published alt text:

> A vertically oriented print depicts a man wearing a hat and coat, looking at us. Depth and shadows are emphasized through the dark shade of the man's clothes and the smudged background.

### 22. Set of Four Armchairs and Settee

- Type: Furniture and woodwork — Decorative Art and Design
- Creator/date: Royal Savonnerie Manufactory, Chaillot Workshops (French, est. 1627); c. 1715
- License: CC0
- Links: [local image](curated_images/1947.183_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1947.183/1947.183_web.jpg) · [museum record](https://www.clevelandart.org/art/1947.183)
- Character count: 164
- Published alt text:

> An armchair features a light-colored floral tapestry with a central image of a raven peering down at a coyote. The wood legs of the armchair are intricately carved.

### 23. Morning Glory with Black

- Type: Painting — American Painting and Sculpture
- Creator/date: Georgia O'Keeffe (American, 1887–1986); 1926
- License: CC0
- Links: [local image](curated_images/1958.42_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1958.42/1958.42_web.jpg) · [museum record](https://www.clevelandart.org/art/1958.42)
- Character count: 177
- Published alt text:

> A vertically oriented oil painting depicts a close-up of the center of a white flower. In the lower right-hand side of the painting, soft slopes of black petals fill the corner.

### 24. Claire Campbell

- Type: Drawing — Drawings
- Creator/date: Édouard Manet (French, 1832–1883); 1876
- License: CC0
- Links: [local image](curated_images/1956.718_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1956.718/1956.718_web.jpg) · [museum record](https://www.clevelandart.org/art/1956.718)
- Character count: 138
- Published alt text:

> A pastel and oil portrait depicts the side profile of a woman facing our left. The subject has pink-tinged light skin and dark brown hair.

### 25. Jar

- Type: Ceramic — Korean Art
- Creator/date: Unknown; 1700s
- License: CC0
- Links: [local image](curated_images/1983.28_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1983.28/1983.28_web.jpg) · [museum record](https://www.clevelandart.org/art/1983.28)
- Character count: 175
- Published alt text:

> A gray-white glazed spherical porcelain jar features narrow, circular rims at the base and mouth. Faint, horizontal lines striate the jar with fine cracks webbing the surface.

### 26. Table Fountain

- Type: Metalwork — Medieval Art
- Creator/date: Unknown; c. 1320–40
- License: CC0
- Links: [local image](curated_images/1924.859_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1924.859/1924.859_web.jpg) · [museum record](https://www.clevelandart.org/art/1924.859)
- Character count: 178
- Published alt text:

> An elaborate golden fountain has three tiers. There are plaques with colorful images of figures on the middle and bottom tiers. Attached to these tiers are waterwheels and bells.

### 27. The Red Kerchief

- Type: Painting — Modern European Painting and Sculpture
- Creator/date: Claude Monet (French, 1840–1926); c. 1868–73
- License: CC0
- Links: [local image](curated_images/1958.39_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1958.39/1958.39_web.jpg) · [museum record](https://www.clevelandart.org/art/1958.39)
- Character count: 135
- Published alt text:

> A vertically oriented oil painting depicts a woman wearing a red kerchief, standing in the snow, and looking at us through glass doors.

### 28. Sleeping Christ Child

- Type: Sculpture — European Painting and Sculpture
- Creator/date: Filippo Parodi (Italian, 1630–1702); c. 1675
- License: CC0
- Links: [local image](curated_images/2018.257_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2018.257/2018.257_web.jpg) · [museum record](https://www.clevelandart.org/art/2018.257)
- Character count: 156
- Published alt text:

> A marble sculpture depicts a sleeping nude young child sprawled on a bed of cloth-covered straw. He faces upward and there are rays emanating from his head.

### 29. Torso of Amenpayom

- Type: Sculpture — Egyptian and Ancient Near Eastern Art
- Creator/date: Unknown; probably 200–100 BCE
- License: CC0
- Links: [local image](curated_images/1948.141_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1948.141/1948.141_web.jpg) · [museum record](https://www.clevelandart.org/art/1948.141)
- Character count: 186
- Published alt text:

> A black rock sculpture depicts the naked torso of a man wearing a kilt. The head, arms, and legs of the sculpture have been broken off. The band of the kilt features carvings of symbols.

### 30. Brush Washer

- Type: Ceramic — Chinese Art
- Creator/date: Unknown; late 1000s–1127
- License: CC0
- Links: [local image](curated_images/1957.40_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1957.40/1957.40_web.jpg) · [museum record](https://www.clevelandart.org/art/1957.40)
- Character count: 99
- Published alt text:

> A wide squat light blue glazed porcelaneous stoneware bowl features a cracked pattern in the glaze.

### 31. Dancers

- Type: Drawing — Drawings
- Creator/date: Edgar Degas (French, 1834–1917); c. 1896
- License: CC0
- Links: [local image](curated_images/1916.1043_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1916.1043/1916.1043_web.jpg) · [museum record](https://www.clevelandart.org/art/1916.1043)
- Character count: 168
- Published alt text:

> A vertically oriented pastel drawing with bright, blended colors depicts three dancers. They all wear identical yellow dresses and are standing and stretching together.

### 32. Sarcophagus

- Type: Sculpture — Greek and Roman Art
- Creator/date: Unknown; c. 100–125 CE
- License: CC0
- Links: [local image](curated_images/1928.856_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1928.856/1928.856_web.jpg) · [museum record](https://www.clevelandart.org/art/1928.856)
- Character count: 178
- Published alt text:

> A large rectangular marble coffin depicts many figures depicting various scenes, including many with the same man holding a short sword, carved in a relief all around the coffin.

### 33. Storage Jar

- Type: Ceramic — Japanese Art
- Creator/date: Unknown; late 1300s–1400s
- License: CC0
- Links: [local image](curated_images/1973.18_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1973.18/1973.18_web.jpg) · [museum record](https://www.clevelandart.org/art/1973.18)
- Character count: 188
- Published alt text:

> A large textured round stoneware jar features a golden-brown glaze. There is a thin design incised around the collar of the jar. The lip of the jar is flared and part of it is chipped off.

### 34. Grapevine

- Type: Painting — Korean Art
- Creator/date: Choe Seok-hwan (Korean, active first half of 19th century); early 1800s
- License: CC0
- Links: [local image](curated_images/2015.510_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2015.510/2015.510_web.jpg) · [museum record](https://www.clevelandart.org/art/2015.510)
- Character count: 176
- Published alt text:

> Eight panels horizontally combine to display a painting of a grapevine in muted colors. The grapevine swirls across the panels with dense foliage and hanging bunches of grapes.

### 35. Dead Blue Roller

- Type: Drawing — Drawings
- Creator/date: Hans Hoffmann (German, 1545/50–1591/92); 1583
- License: CC0
- Links: [local image](curated_images/1946.217_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1946.217/1946.217_web.jpg) · [museum record](https://www.clevelandart.org/art/1946.217)
- Character count: 130
- Published alt text:

> A hyperrealistic, vertically oriented watercolor painting depicts a dead bird on the ground. The bird has blue and brown feathers.

### 36. Figure of a Warrior

- Type: Metalwork — Art of the Americas
- Creator/date: Unknown; after 1325 (thermoluminescence date of casting core, 1345–1575)
- License: CC0
- Links: [local image](curated_images/1984.37_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1984.37/1984.37_web.jpg) · [museum record](https://www.clevelandart.org/art/1984.37)
- Character count: 188
- Published alt text:

> A small golden sculpture depicts a standing figure holding weapons in their right hand and a shield in their left hand. The figure wears jewelry and the front of their chest is carved out.

### 37. Satyress and Child

- Type: Sculpture — Modern European Painting and Sculpture
- Creator/date: Clodion (French, 1738–1814); 1803
- License: CC0
- Links: [local image](curated_images/1963.251_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1963.251/1963.251_web.jpg) · [museum record](https://www.clevelandart.org/art/1963.251)
- Character count: 187
- Published alt text:

> A circular terracotta relief depicts a nude woman with horse legs and hooves holding a baby with horse legs and hooves. On our right side of the circular disc is a carving of a grapevine.

### 38. Wine Cup with Children at Play

- Type: Ceramic — Chinese Art
- Creator/date: Unknown; 1465–87
- License: CC0
- Links: [local image](curated_images/1957.61_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1957.61/1957.61_web.jpg) · [museum record](https://www.clevelandart.org/art/1957.61)
- Character count: 96
- Published alt text:

> A white porcelain cup features faded colorful paintings of scenes of children playing in nature.

### 39. Mandala of the Three Shrines at Kumano

- Type: Painting — Japanese Art
- Creator/date: Unknown; 1300s
- License: CC0
- Links: [local image](curated_images/1953.16_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1953.16/1953.16_web.jpg) · [museum record](https://www.clevelandart.org/art/1953.16)
- Character count: 195
- Published alt text:

> A hanging scroll depicts three compounds with brown-roofed buildings in a stacked composition. Each compound, placed in a mountainous setting, is laid out in a rectangle around a large courtyard.

### 40. Sleep and Death Cista Handle

- Type: Sculpture — Greek and Roman Art
- Creator/date: Unknown; 400–375 BCE
- License: CC0
- Links: [local image](curated_images/1945.13_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1945.13/1945.13_web.jpg) · [museum record](https://www.clevelandart.org/art/1945.13)
- Character count: 127
- Published alt text:

> A small bronze sculpture depicts two winged deities dressed like warriors carrying a lifeless naked body sideways between them.

### 41. Jonah Swallowed

- Type: Sculpture — Medieval Art
- Creator/date: Unknown; 280–90 CE
- License: CC0
- Links: [local image](curated_images/1965.237_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1965.237/1965.237_web.jpg) · [museum record](https://www.clevelandart.org/art/1965.237)
- Character count: 182
- Published alt text:

> A marble sculpture depicts a sea monster swallowing a man headfirst. The sea monster has a canine head and serpent body. Its mouth is open with just the man's legs outside the mouth.

### 42. Jar with Dragon and Clouds Design

- Type: Ceramic — Korean Art
- Creator/date: Unknown; late 1600s
- License: CC0
- Links: [local image](curated_images/1986.69_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1986.69/1986.69_web.jpg) · [museum record](https://www.clevelandart.org/art/1986.69)
- Character count: 149
- Published alt text:

> A spherical white porcelain jar features a loosely painted dark brown dragon that wraps around it. Surrounding the dragon are squiggle line drawings.

### 43. Funeral for Marie-Thérèse of Spain, Dauphine of France, in the Church of Nôtre Dame, Paris, on November 24, 1746

- Type: Drawing — Drawings
- Creator/date: Charles-Nicolas Cochin (French, 1715–1790); c. 1746
- License: CC0
- Links: [local image](curated_images/2000.2_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/2000.2/2000.2_web.jpg) · [museum record](https://www.clevelandart.org/art/2000.2)
- Character count: 174
- Published alt text:

> An elaborate, vertically oriented drawing depicts a funeral scene in a Gothic cathedral. There are many people gathered around an ornate arched structure containing a coffin.

### 44. Swift Bull from the Ten Swift Bulls

- Type: Painting — Japanese Art
- Creator/date: Unknown; late 1200s–early 1300s
- License: CC0
- Links: [local image](curated_images/1952.286_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1952.286/1952.286_web.jpg) · [museum record](https://www.clevelandart.org/art/1952.286)
- Character count: 154
- Published alt text:

> A hanging scroll features two vertical straps at the top and a painting in the center. The painting is of a bull, painted with black ink, facing our left.

### 45. Head of a Tahitian Woman

- Type: Drawing — Drawings
- Creator/date: Paul Gauguin (French, 1848–1903); 1891
- License: CC0
- Links: [local image](curated_images/1949.439_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1949.439/1949.439_web.jpg) · [museum record](https://www.clevelandart.org/art/1949.439)
- Character count: 144
- Published alt text:

> A graphite portrait depicts a woman's head with a light outline of her upper torso. She has long dark hair, with one side tucked behind her ear.

### 46. Water Ewer for Rituals (Kundika) with Incised Parrot Design

- Type: Ceramic — Korean Art
- Creator/date: Unknown; 1100s
- License: CC0
- Links: [local image](curated_images/1921.631_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1921.631/1921.631_web.jpg) · [museum record](https://www.clevelandart.org/art/1921.631)
- Character count: 141
- Published alt text:

> A celadon, a gray-green glaze, jug features a long, narrow neck and a short, capped spout. On the body is a carved image of a flying phoenix.

### 47. Torso of a Kouros

- Type: Sculpture — Greek and Roman Art
- Creator/date: Unknown; c. 550 BCE
- License: CC0
- Links: [local image](curated_images/1953.125_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1953.125/1953.125_web.jpg) · [museum record](https://www.clevelandart.org/art/1953.125)
- Character count: 164
- Published alt text:

> A life-size marble sculpture with lightly chipped areas depicts a nude man's torso and top parts of the thighs. The head, arms, and rest of the legs are broken off.

### 48. Mourner from the Tomb of John the Fearless and Margaret of Bavaria

- Type: Sculpture — Medieval Art
- Creator/date: Jean de la Huerta (Spanish, active Burgundy, 1431–62); 1443–45
- License: CC0
- Links: [local image](curated_images/1940.129_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1940.129/1940.129_web.jpg) · [museum record](https://www.clevelandart.org/art/1940.129)
- Character count: 160
- Published alt text:

> A small beige sculpture depicts a man dressed in a monk's robe, looking up to our right. His right hand holds a slightly open book while his left hand is empty.

### 49. The Illustrated Miraculous Origins of the Yūzū Nenbutsu School

- Type: Painting — Japanese Art
- Creator/date: Unknown; early 1300s
- License: CC0
- Links: [local image](curated_images/1956.87_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1956.87/1956.87_web.jpg) · [museum record](https://www.clevelandart.org/art/1956.87)
- Character count: 120
- Published alt text:

> A handscroll is filled with writings in Japanese next to drawings, depicting a story with many smaller scenes of people.

### 50. Landscape with Wheelbarrow

- Type: Drawing — Drawings
- Creator/date: Vincent van Gogh (Dutch, 1853–1890); 1883
- License: CC0
- Links: [local image](curated_images/1958.30_web.jpg) · [source image](https://openaccess-cdn.clevelandart.org/1958.30/1958.30_web.jpg) · [museum record](https://www.clevelandart.org/art/1958.30)
- Character count: 112
- Published alt text:

> A horizontally oriented watercolor depicts a field of greenery under a gray sky with a wheelbarrow to our right.

## Research notes and exclusions

The revised search inspected 362 primary images whose published metadata identified them as human-reviewed. Sixty-two of those descriptions met the 200-character maximum; 50 were selected for variety while preserving all qualifying entries from the earlier report.

Web searches also identified several potentially useful sources that were not used for the final 50:

- [Cooper Hewitt's Guidelines for Image Description](https://www.cooperhewitt.org/cooper-hewitt-guidelines-for-image-description/) supplies unusually strong, specialist-written examples. However, many of its example images are modern copyrighted works or site assets without a clearly stated CC0/CC BY license.
- [DIAGRAM Center's Image Description Guidelines](https://diagramcenter.org/table-of-contents-2.html) are grounded in accessibility research and include many image types. Many underlying examples, however, are copyrighted textbook figures, and several demonstrate context-dependent descriptions—the kind the prompt asked to avoid.
- The expert-rated [Composing alt text using large language models](https://data.mendeley.com/datasets/szh5zhpgxh/1) dataset explicitly scores descriptions for usefulness, reliability, completeness, accuracy, and literacy. Most original images are copyrighted press photographs or protected logos, so they did not meet the reuse requirement.
- Ordinary museum alt text, web-scraped caption datasets, and descriptions that passed only an automated score were excluded.

The Cleveland source ultimately provided the clearest intersection of accessibility expertise, explicit human review, context-independent visual description, individual image links, clear CC0 status, and enough variety for a balanced set of 50.

## Source documentation

- [Making Art Accessible to All](https://www.clevelandart.org/making-art-accessible-all) — describes the disabled-expert-led collaboration, style-guide development, staff writing and feedback, image-level descriptions, publishing workflow, and human-review fields.
- [Cleveland Museum of Art Open Access](https://www.clevelandart.org/open-access) — explains CC0 reuse and how Open Access records are identified.
- [Cleveland Museum of Art Open Access API](https://openaccess-api.clevelandart.org/) — documents the collection fields and CC0 filter.
- [CMA Collection Online Alt-Text Style Guide](https://static.sched.com/hosted_files/mcn2024/15/CMA%20Collection%20Online%20Alt-Text%20Style%20Guide.pdf) — the collection-specific writing standard.
- [Cooper Hewitt Guidelines for Image Description](https://www.cooperhewitt.org/cooper-hewitt-guidelines-for-image-description/) — an influential specialist source consulted during the broader search.
- [DIAGRAM Center Image Description Guidelines](https://diagramcenter.org/table-of-contents-2.html) — an accessibility-research source consulted for non-art image types.

## Clarifications supplied

Acceptable rights: CC0/public domain or CC BY with attribution. Merely appearing on a GLAM+ organization's website was not treated as proof of quality. Preference was given to sources explicitly concerned with producing good alt text, and a shared quality reason could apply to multiple links from the same source.


## Prompt

### Original prompt

```text
Goal: Find 50 images with alt-text that has been determined, by knowledgeable humans to be "good".

Context:

- We want to use the images in a project, so they must have very permissive usage/creative-commons licensing.

- We know good alt-text involves understanding the context in which an image appears. That is not possible for our use-case -- so examples that show why alt-text is good -- because it conveys information relevant to the context -- should be avoided for this search.

Tasks:

- Perform web searches to try to find images of different types with "good" alt-text.

- Ask me one to two clarifying questions before starting.

- Save the results to ~/Desktop/good_alt_text_results.md`, with this prompt near the bottom.
```

### Update

```text
Thanks. I really like that format. Two additions:

- I've heard that standard max-lengths for alt-text used to be 125 characters, but now 250-characters is often the max. Let's say that for this sample 200-characters is the max.

- Please add a character-count bullet to the info for each image.

Feel free to keep all the current images where the alt-text is less than 200-characters. 

Please save the updated info at the same path: `~/Desktop/good_alt_text_results.md`. Thanks.
```

---
